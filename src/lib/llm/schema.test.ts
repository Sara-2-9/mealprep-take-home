...
/** Minimal valid plan: 7 days × 3 meals, cheapest real catalog products. */
    ...
    days: WEEK_DAY_NAMES.map((day) => ({
      ...
      meals: [
        ...
        },
        {
          ...
          steps: ["Boil pasta", "Cook sauce", "Serve", "Enjoy"],
        },
      ],
    })),
  };
...
describe("weeklyPlanSchema", () => {
  test("accepts a valid 7-day plan with 3 meals each", () => {
  ...
  test("rejects days with fewer than 3 meals", () => {
    const plan = makeValidPlan();
    const twoMeals = structuredClone(plan);
    twoMeals.days[0].meals = twoMeals.days[0].meals.slice(0, 2);
    expect(weeklyPlanSchema.safeParse(twoMeals).success).toBe(false);
  });

  test("rejects grams below 1 and missing grams", () => {
    const plan = makeValidPlan();
    const zeroGrams = structuredClone(plan);
    zeroGrams.days[0].meals[0].ingredients[0].grams = 0;
    expect(weeklyPlanSchema.safeParse(zeroGrams).success).toBe(false);

    const noGrams = JSON.parse(JSON.stringify(plan));
    delete noGrams.days[0].meals[0].ingredients[0].grams;
  ...
  test("rejects meals with fewer than 2 ingredients or 3 steps", () => {
    ...
    fewIngredients.days[0].meals[0].ingredients = [
      fewIngredients.days[0].meals[0].ingredients[0],
    ];
    expect(weeklyPlanSchema.safeParse(fewIngredients).success).toBe(false);

    const fewSteps = structuredClone(plan);
    fewSteps.days[0].meals[0].steps = ["Cook", "Serve"];
  ...
  test("caps recipes at 10 ingredients and 8 steps (output slimming)", () => {
    ...
    manyIngredients.days[0].meals[0].ingredients = Array.from(
      { length: 11 },
      (_, i) => ({
    ...
    manySteps.days[0].meals[0].steps = Array.from({ length: 9 }, (_, i) => `Step ${i}`);
    expect(weeklyPlanSchema.safeParse(manySteps).success).toBe(false);
  });

  test("rejects meals with invalid meal type", () => {
    const plan = makeValidPlan();
    (plan.days[0].meals[0] as any).type = "snack";
    expect(weeklyPlanSchema.safeParse(plan).success).toBe(false);
  });
});
